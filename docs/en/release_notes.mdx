...
- **Security.** All PostgreSQL operand images were rebuilt on patched bases and dependencies — Spilo, pg-agent, RDS Operator, Pgpool-II, and the PostgreSQL exporter — and the Go toolchain, `golang.org/x/net`, and `golang.org/x/text` were updated. Scanning the released images shows the vulnerability count reduced from 672 in v4.3.3 to 100 in v4.3.4, with **no High or Critical findings remaining**. See the accompanying security advisory for the full list of addressed CVEs.

### Fixed Issues

- **A cluster demoted to standby could not rejoin its cross-cluster pair.** After a planned cross-cluster switchover — patching a primary to a standby rather than deleting and recreating it — the demoted cluster kept replicating against the topology that existed before the switchover and never reached the newly promoted primary. Patroni retried against the `<cluster>-xcr` endpoint in a connect-timeout loop without reporting the cause, and because a standby cluster has no automatic reinitialization fallback, the cluster could remain stuck instead of recovering on its own. The operator now resolves peer information from the peer cluster when demoting a primary, evaluates replication-metadata staleness against the configuration being applied rather than the one still running, and logs an explicit warning when a demotion can never rewind because both `wal_log_hints` and data checksums are disabled. Deleting and recreating the cluster is no longer required to recover it.

{/* release-notes-for-bugs?template=mw-pg-v4.3.4-fixed&project=Middleware */}

### Known Issues

- **Logical backups are unavailable in disconnected environments.** Enabling `enableLogicalBackup` schedules a job that runs an image which is not included in the plugin package. In environments without access to an external image registry, the job fails with `ImagePullBackOff`. Clusters that do not enable logical backups are unaffected.
